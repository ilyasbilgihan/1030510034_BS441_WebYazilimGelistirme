...
export default function Game({ gameMode, setGameMode }) {
  ...
  const updateScores = (newMoves) => {
    const freq = calculateFrequency(
      gameMode == "1v1" ? newMoves.slice(0, -1) : newMoves
    );
    const tempScores = [];
      ...
      <>
        ...
        >
          ...
          <div>
            ...
            {gameMode == "1v1v1" && (
              <Player name="Computer 2" move={moves[2]} suffix="-r" />
            )}
          </div>
          {victory.length != 0 && (
            <>
              {victory.length == 1 ? (
                <div>
                  {victory[0] != 0
                    ? "You Lose | Winner: " + PLAYERS[victory[0]]
                    : "You Win"}
                </div>
              ) : (
                <div>
                  Draw between | {victory.map((v) => PLAYERS[v]).join(", ")}
                </div>
              )}
              <div>
